...
#dependencies: python3, bbmap (packaged), Biopython, PySAM, twobitreader
inputs = {}
# inputs:

inputs["userandomIS"] = 0  # 0 #useful for generating a random control data set chromosome matched to query set.
inputs["userandomSEQS"] = 0  # generate random sequence data, usefull for calculating false positive rate when mapping reads w/
# bowtie

# tasks:
inputs["getseqs"] = 1  # 1#get sequences from either entrez or local TwoBit genome around locations indicated by
# genome_location_csv
inputs["compressreads"] = 1  # remove duplicate reads and reads shifted +/- 1 nt, number of reads are compressed in csv and fasta
inputs["getannotations"] = 1  # 1 # use annotate.py module to map insertion sites to anotations

# parameters
inputs["barcode"] = ''  # 5' barcode sequence 05:ATCTGCGACG
inputs["lwindow"] = 50  # window on either side of indicated nt location to return
inputs["rwindow"] = 50
inputs["samwindow"] = 0  # 60
inputs["sequencesource"] = 'LOCAL'  # get sequences from 'LOCAL' TwoBit genome or 'REMOTE' entrez server
inputs["minimum_read_len"] = 25  # minimum length of a read to try mapping. 25 was found to usually avoid any false positives
# in read sets of 200k reads
inputs["primer5"] = ''  # 5' primer sequenc to remove from reads
inputs["primer3"] = ''  # 3' primer sequence to remove from reads
inputs["trim5"] = 0  # additional (non-genomic) nts to trim off of 3' end of reads #21
inputs["trim3"] = 0  # additional (non-genomic) nts to trim off of 5' end of reads (starts with TA) #16
inputs["featurenames"] = ['intron', 'exon', 'coding exon', 'transcript',
                'Transcriptional_Start_Site']  # feature names found in feature files to map reads to
inputs["featuredist"] = [False, False, False, False,
               True]  # weather to map distance of each read, or only whether reads overlap with feature
inputs["distance"] = [1000, 2000, 4000, 8000, 16000, 32000, 64000]  # distance in bp to be considered close to feature

# outputs:
inputs["write_csv"] = 1  # write csv (only relevant if reading from sam file)
inputs["writeFASTA"] = 1  # 1#write a fasta file of all sequences around IS
inputs["writelogo"] = 1  # 1#create a logo image of consensus sequence # requires getseqs and writeFASTA to be on


# reference file locations
inputs["chromosome_ids"] = './reference_datasets/chromosomes.csv'
inputs["bbmerge_location"] = './bins/bbmap/bbmerge-auto.sh'
inputs["bowtie_location"] = '/btapps/miniconda3/bin/bowtie2' #'bowtie2'
inputs["bowtie_index_file"] = './reference_datasets/genomes/GRCh38.fna.bowtie_index/GCA_000001405.15_GRCh38_no_alt_analysis_set.' \
              'fna.bowtie_index'
inputs["weblogo_location"] = 'weblogo'  # in PATH
inputs["twobit_location"] = './scripts/TwoBitToFa'
inputs["twobit_genome_location"] = './reference_datasets/genomes/GRCh38.2bit'
inputs["bbduk_location"] = './bins/bbmap/bbduk.sh'
inputs["annotations"] = ['./reference_datasets/annotations/refseq.introns.bed',
...
def main(args):
    ...
    warnings = []
    # sanity checks:
    # check that java is installed (required for bbmap commands)
    ...
    if inputtype == "fastq":
        ...
        bowtiecommand = f'{bowtie_location} --phred33 -p 4 -x {bowtie_index_file} -U {trimmedfastq} --no-unal ' \
                            f'-S {sam_file}'
        bbmap_location = "./bins/bbmap"
        bbmapindex_location = "ref=./reference_datasets/genomes/ncbi-genomes-2023-01-06/GCF_000001405.40_GRCh38.p14_genomic.fna path=./reference_datasets/genomes/"
        bbmapcommand = f'{bbmap_location} in={trimmedfastq} ref={bbmapindex_location} out={sam_file} -Xmx30g'
