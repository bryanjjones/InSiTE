...
if __name__ == "__main__":
    ...
    ap.add_argument('-z', '--compress_reads', action='store_true',
                    help='compress duplicate reads and reads shifted +/- 1nt, number of reads are compressed in csv, fasta, and mapping outputs')
    ...
    ap.add_argument('--min', default=25, type=int,
                    help='minimum length of a read to try mapping. default (25) will usually avoid any false positives in read sets of 200k reads')
    ...
    ap.add_argument('--feature', action='append', metavar='intron/exon/transcript/TSS/etc',
                    help='feature names found in feature files to map reads to, e.g. "exon"')  # ['intron', 'exon','codingexon','transcript','TSS'] #feature names found in feature files to map reads to
    ap.add_argument('--dist', action='append', metavar='True/False',
                    help='weather to map distance of each read, or only whether reads overlap with feature. same number of distance variables must be given as features.')
    ...
    ap.add_argument('--bowtieindex', metavar='/path/to/bowtieindex',
                    default='./refrence_datasets/genomes/GRCh38.fna.bowtie_index/GCA_000001405.15_GRCh38_no_alt_analysis_set.fna.bowtie_index')
    ...
    if args.feature:
        if not len(args.feature) == len(args.dist) == len(args.annotations):
            logging.critical(
                'must provide same number of features as distance and annotation files as they correspond to each other')
            sys.exit(
                'must provide same number of features as distance and annotation files as they correspond to each other')
...
if (writeFASTA == 1 or writelogo == 1) and getseqs == 0:  # or writevepfile ==1 or or getannotations==1
    ...
    sys.exit(
        colorama.Fore.RED + "Can't write a fasta file or logo file, or get annotations without getting sequences.")  # , vep file,
...
if getannotations and not len(annotations) == len(featurenames) == len(featuredist):
    ...
    sys.exit(
        colorama.Fore.RED + "If getting annotations, length of annotation files, feature names, and feature distance must be the same")
...
if getannotations and len:
    with open(annotationsfile, 'w', newline='') as output_file:
        ...
        for i in range(len(featurenames)):
            if featuredist[i]:
                print(
                    colorama.Style.RESET_ALL + f'mapping insertion site distances to ' + colorama.Fore.YELLOW + f'{featurenames[i]}' + colorama.Style.RESET_ALL + f' in ' + colorama.Fore.YELLOW + f'{annotations[i]}' + colorama.Style.RESET_ALL)
                logging.info(
